// src/components/AdminDashBord.jsx
...
function AdminDashBord() {
  ...
  const hnadleTodayChekout = () => {
    navigate("/all-checkout");
  };
  ...
  const renderStatusBadge = (status) => {
    ...
    if (lowerStatus === "active") {
      ...
      icon = "✅";
    } else if (lowerStatus === "auto-finished" || lowerStatus === "auto-finish") {
  ...
  useEffect(() => {
    ...
    // updateCheckin -> either update or remove if not active
    const handleUpdateCheckin = (data) => {
      if (!data) return;
      // if status changed to not active, remove from admin list (auto-finished or checked-out)
      if (data.status && data.status.toLowerCase() !== "active") {
        setCheckins((prev) => prev.filter((i) => i._id !== data._id));
        return;
      }
    ...
    // employeeCheckedOut -> small payload { checkInId }
    const handleRemove = (data) => {
      const id = data?.checkInId || data?._id || data?.id;
      if (!id) return;
      setCheckins((prev) => prev.filter((i) => i._id !== id));
    };
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 text-base">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-5 py-3 text-left text-sm font-bold text-gray-700 uppercase tracking-wider min-w-[120px]">Employee</th>
                    <th className="px-5 py-3 text-left text-sm font-bold text-gray-700 uppercase tracking-wider min-w-[250px]">Email</th>
                    <th className="px-5 py-3 text-left text-sm font-bold text-gray-700 uppercase tracking-wider min-w-[80px]">Image</th>
                    <th className="px-5 py-3 text-left text-sm font-bold text-gray-700 uppercase tracking-wider min-w-[180px]">Check-In Time</th>
                    <th className="px-5 py-3 text-left text-sm font-bold text-gray-700 uppercase tracking-wider min-w-[300px]">Location</th>
                    <th className="px-5 py-3 text-left text-sm font-bold text-gray-700 uppercase tracking-wider min-w-[140px]">Status</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-100">
                  {checkins.map((c) => (
                    <tr
                      ...
                      <td className="px-5 py-4 text-gray-600 whitespace-nowrap text-sm">{c.user?.email || "N/A"}</td>
                      ...
                      <td className="px-5 py-4 text-gray-600 whitespace-nowrap text-sm">{new Date(c.checkInTime).toLocaleString()}</td>
                      <td className="px-5 py-4 text-gray-600 text-sm">{renderLocation(c.location)}</td>
                      <td className="px-5 py-4 whitespace-nowrap">{renderStatusBadge(c.status)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            {checkins.length === 0 && !loading && <p className="p-8 text-center text-gray-500">No check-in records found.</p>}
          </div>
        )}
      </main>
    </div>
